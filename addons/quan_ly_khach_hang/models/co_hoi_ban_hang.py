...
class CoHoiBanHang(models.Model):
    ...
    def write(self, vals):
        ...
        old_stages = {r.id: r.giai_doan for r in self}
        result = super().write(vals)

        if vals.get('giai_doan') == 'thang' and create_order_records:
            create_order_records._auto_create_don_hang_on_win()

        # Gửi email khi cơ hội thắng hoặc thua
        new_stage = vals.get('giai_doan')
        if new_stage in ('thang', 'thua'):
            for rec in self:
                if old_stages.get(rec.id) != new_stage:
                    rec._send_opportunity_notification(new_stage)

        return result

    def _send_opportunity_notification(self, stage):
        """Gửi email thông báo kết quả cơ hội cho nhân viên phụ trách"""
        service = self.env['email.notification.service']
        self.ensure_one()
        email = self.nhan_vien_phu_trach_id.email if self.nhan_vien_phu_trach_id else False
        if not email:
            return
        if stage == 'thang':
            title = f'Cơ hội "{self.ten_co_hoi}" đã THẮNG!'
            greeting = f'Xin chào {self.nhan_vien_phu_trach_id.ho_va_ten},<br>Cơ hội bán hàng đã <b style="color:green;">thắng</b>. Chúc mừng!'
        else:
            title = f'Cơ hội "{self.ten_co_hoi}" đã thua'
            greeting = f'Xin chào {self.nhan_vien_phu_trach_id.ho_va_ten},<br>Cơ hội bán hàng đã <b style="color:red;">thua</b>.'
        lines = [
            ('Mã cơ hội', self.ma_co_hoi),
            ('Tên cơ hội', self.ten_co_hoi),
            ('Khách hàng', self.khach_hang_id.ten_khach_hang if self.khach_hang_id else ''),
            ('Giá trị dự kiến', f'{self.gia_tri_du_kien:,.0f} VNĐ'),
        ]
        body = service._build_email_html(title, greeting, lines, footer_name='Phòng Kinh doanh')
        ok, msg = service.send_email(email, title, body, from_name='Hệ thống CRM')
