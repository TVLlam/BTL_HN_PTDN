...
class KhachHang(models.Model):
    ...
    partner_id = fields.Many2one(
        'res.partner',
        string="Đối tác ERP",
        help="Đồng bộ khách hàng với danh bạ chuẩn để dùng chung cho Document/HR/Email",
        ondelete='set null',
    )
    ...
    def write(self, vals):
        ...
        self._sync_partner_master()
        return res

    def _sync_partner_master(self):
        """Đồng bộ khách hàng sang res.partner để các module khác dùng chung."""
        for rec in self:
            partner = rec.partner_id
            # Ưu tiên tìm theo mã số thuế rồi tới email/phone
            if not partner and rec.ma_so_thue:
                partner = rec.env['res.partner'].sudo().search([('vat', '=', rec.ma_so_thue)], limit=1)
            if not partner and rec.email:
                partner = rec.env['res.partner'].sudo().search([('email', '=', rec.email)], limit=1)
            if not partner and rec.dien_thoai:
                partner = rec.env['res.partner'].sudo().search([('phone', '=', rec.dien_thoai)], limit=1)
            if not partner:
                partner_vals = {
                    'name': rec.ten_khach_hang,
                    'company_type': 'company' if rec.loai_khach_hang == 'doanh_nghiep' else 'person',
                    'street': rec.dia_chi,
