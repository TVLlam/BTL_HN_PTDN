    ...
    def action_xac_nhan(self):
        ...
        for record in self:
            if record.trang_thai == 'nhap':
                ...
                record._send_order_notification('xac_nhan')
    ...
    def action_hoan_thanh(self):
        ...
        for record in self:
            ...
            if record.trang_thai == 'dang_thuc_hien':
                ...
                record._send_order_notification('hoan_thanh')
                record._send_giao_hang_email()

    def _send_order_notification(self, action):
        ...
        if not email:
            return
        if action == 'xac_nhan':
            title = f'Đơn hàng {self.ma_don_hang} đã được xác nhận'
            greeting = f'Kính gửi {self.khach_hang_id.ten_khach_hang},<br>Đơn hàng của quý khách đã được <b style="color:green;">xác nhận</b>.'
        else:
            title = f'Đơn hàng {self.ma_don_hang} đã hoàn thành'
            greeting = f'Kính gửi {self.khach_hang_id.ten_khach_hang},<br>Đơn hàng của quý khách đã <b style="color:green;">hoàn thành</b>.'
        lines = [
            ('Mã đơn hàng', self.ma_don_hang),
            ('Tên đơn hàng', self.ten_don_hang),
            ('Ngày đặt', str(self.ngay_dat)),
            ('Tổng giá trị', f'{self.tong_gia_tri:,.0f} VNĐ'),
        ]
