...
class VanBanDen(models.Model):
    ...
    partner_id = fields.Many2one(
        ...
        store=True,
        readonly=False,
        help="Đồng bộ với danh bạ chuẩn để báo cáo liên module",
    )
    ...
    so_cong_van_id = fields.Many2one('so_cong_van', string="Sổ công văn", ondelete='set null',
                                      domain="[('loai_so', '=', 'den'), ('trang_thai', '=', 'mo')]")
    ...
    def _send_assignment_notification(self):
        """Gửi email thông báo cho người xử lý khi được phân công văn bản đến"""
        self.ensure_one()
        email = self.nguoi_xu_ly_id.email if self.nguoi_xu_ly_id else False
        if not email:
            return
        service = self.env['email.notification.service']
        title = f'Bạn được phân công xử lý văn bản: {self.so_ky_hieu}'
        greeting = f'Xin chào {self.nguoi_xu_ly_id.ho_va_ten},<br>Bạn vừa được phân công xử lý một văn bản đến.'
        lines = [
            ('Số/Ký hiệu', self.so_ky_hieu),
            ('Nơi ban hành', self.noi_ban_hanh or ''),
            ('Trích yếu', self.trich_yeu or ''),
            ('Độ khẩn', dict(self._fields['do_khan'].selection).get(self.do_khan, '') if self.do_khan else ''),
            ('Hạn xử lý', str(self.han_xu_ly) if self.han_xu_ly else 'Không có'),
        ]
