...
class VanBanDi(models.Model):
    ...
    partner_id = fields.Many2one(
        ...
        store=True,
        readonly=False,
        help="Đồng bộ với danh bạ chung để gắn văn bản đi vào báo cáo ERP",
    )
    ...
    # Sổ công văn
    so_cong_van_id = fields.Many2one('so_cong_van', string="Sổ công văn", ondelete='set null',
                                      domain="[('loai_so', '=', 'di'), ('trang_thai', '=', 'mo')]")
    ...
    @api.depends('khach_hang_id.partner_id')
    def _compute_partner(self):
        for rec in self:
            rec.partner_id = rec.khach_hang_id.partner_id.id if rec.khach_hang_id and rec.khach_hang_id.partner_id else False
    ...
    def _send_notification_email(self):
        ...
        self.ensure_one()
        email = self.khach_hang_id.email if self.khach_hang_id else False
        if not email:
            self.message_post(body="Không thể gửi email: Khách hàng không có địa chỉ email")
            return

        service = self.env['email.notification.service']
        title = f'Thông báo: Văn bản {self.so_ky_hieu} đã được phê duyệt'
        greeting = f'Kính gửi {self.khach_hang_id.ten_khach_hang},<br>Văn bản của quý khách đã được <b style="color:green;">phê duyệt</b> và gửi đi.'
        lines = [
            ('Số/Ký hiệu', self.so_ky_hieu),
            ('Ngày văn bản', self.ngay_van_ban.strftime('%d/%m/%Y') if self.ngay_van_ban else ''),
            ('Người ký', self.nguoi_ky or ''),
            ('Trích yếu', self.trich_yeu or ''),
            ('Độ khẩn', dict(self._fields['do_khan'].selection).get(self.do_khan, self.do_khan) if self.do_khan else ''),
        ]
        body = service._build_email_html(title, greeting, lines, footer_name='Phòng Quản lý Văn bản')
        ok, msg = service.send_email(email, title, body, from_name='Hệ thống QLVB')
