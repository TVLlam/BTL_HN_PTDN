...
class ChatbotService(models.AbstractModel):
    ...
    DEFAULT_MODEL_KEY = 'gemini_flash'
...
"""
    ...
    def _call_gemini(self, api_key, system_prompt, user_question, model_name):
        ...
        _logger.info("Calling Gemini REST API model: %s", model_name)
        ...
        try:
            ...
            _logger.error("Gemini API error: %s - %s", e.code, error_body)
            if e.code == 429:
                raise Exception("Gemini báo quá giới hạn hoặc hết quota. Vui lòng thử lại sau.")
            if e.code == 400:
                raise Exception(f"API key Gemini không hợp lệ hoặc request sai. Chi tiết: {error_body[:200]}")
            if e.code == 403:
                raise Exception("API key Gemini bị từ chối. Kiểm tra lại key hoặc bật Generative Language API.")
            if e.code == 404:
                raise Exception(f"Model Gemini không tồn tại: {model_name}.")
            raise Exception(f"Lỗi Gemini API: {e.code}")
        except urllib.error.URLError as e:
            _logger.error("Gemini connection error: %s", str(e))
            raise Exception("Không thể kết nối đến Gemini API")
